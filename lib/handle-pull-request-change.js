...
async function handlePullRequestChange(context) {
	...
	function getDescription() {
		...
		const commitsPart =
			requireCommits !== 'none'
				? `${hasSemanticCommits ? '✅' : '⛔️'} ${requireCommits === 'all' ? 'all' : 'some'} commits  — `
				: '';
		const brPrPart = requirePrBreakingChangeColon ? `${bodyBreakingHasColon ? '✅' : '⛔️'} BC colon (PR)  — ` : '';
		const brCommitPart = requireCommitBreakingChangeColon ? `${breakingCommitMissingColon ? '✅' : '⛔️'} BC colon (commit)  —` : '';

		return `${titlePart}${commitsPart}${prBodyHasBreaking ? brPrPart : ''}${hasBreakingCommit ? brCommitPart : ''}`;
	}
